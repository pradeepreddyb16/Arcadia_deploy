...
############### ADMIN ###############
    ...
    def addadmin(uname, uemail, umobile, upass, uotp, uregdate, urole, ulogindate, uotpdate, status):
        ...
        cur.execute(sql, [uname, uemail, umobile, upass, uotp,
                    uregdate, urole, ulogindate, uotpdate, status])
        mysql.connection.commit()
        cur.close()
...
############### STORE ###############
    ...
    def addStore(store_no, code, uid, name, open_date, city, state, region, type, status, foot_m2, foot_ft2, added_date, active_status, tracker1, tracker2, tracker3, tracker4, tracker5, tracker6, tracker7, tracker8, tracker9, tracker10):
        ...
        cur.execute(sql, [store_no, code, uid, name, open_date, city, state, region, type, status, foot_m2, foot_ft2, added_date,
    ...
    def updateStore(store_no, code, name, open_date, city, state, region, typee, status, foot_m2, foot_ft2,  active_status, tracker1, tracker2, tracker3, tracker4, tracker5, tracker6, tracker7, tracker8, tracker9, tracker10, idd):
        ...
        cur.execute(sql, [store_no, code, name, open_date, city, state, region, typee, status, foot_m2, foot_ft2, active_status,
                    tracker1, tracker2, tracker3, tracker4, tracker5, tracker6, tracker7, tracker8, tracker9, tracker10, idd])
        mysql.connection.commit()
        cur.close()
...
# QUARTERLY VIEW1
    ...
    def quaterlydata1(month1):
        ...
        sql = "SELECT store_no, store_code, store_name, city, state, region,SUM(CASE WHEN month = '"+month1+"' THEN CAST((elec___kwh)AS DECIMAL(10,2)) END) AS month1_elec_kwh,SUM(CASE WHEN month = '"+month1+"' THEN CAST((dg___kwh)AS DECIMAL(10,2)) END) AS month1_dg_kwh,SUM(CASE WHEN month = '"+month1+"' THEN CAST((hvac___kwh)AS DECIMAL(10,2)) END) AS month1_hvac_kwh,SUM(CASE WHEN month = '"+month1 + \
            "' THEN CAST((r22___kg)AS DECIMAL(10,2)) END) AS month1_r22_kg,SUM(CASE WHEN month = '"+month1+"' THEN CAST((r404___kg)AS DECIMAL(10,2)) END) AS month1_r404_kg,SUM(CASE WHEN month = '"+month1 + \
            "' THEN CAST((r407___kg)AS DECIMAL(10,2)) END) AS month1_r407_kg,SUM(CASE WHEN month = '"+month1 + \
            "' THEN CAST((other___kg)AS DECIMAL(10,2)) END) AS month1_other_kg FROM approved_month WHERE month IN (%s) GROUP BY store_code ORDER BY store_code; "
...
# QUARTERLY VIEW2
    ...
    def quaterlydata3(month1, month2, month3):
        ...
        sql = "SELECT store_no, store_code, store_name, city, state, region,SUM(CASE WHEN month = '"+month1+"' THEN CAST((elec___kwh)AS DECIMAL(10,2)) END) AS month1_elec_kwh,SUM(CASE WHEN month = '"+month2+"' THEN CAST((elec___kwh)AS DECIMAL(10,2)) END) AS month2_elec_kwh,SUM(CASE WHEN month = '"+month3+"' THEN CAST((elec___kwh)AS DECIMAL(10,2)) END) AS month3_elec_kwh,SUM(CASE WHEN month = '"+month1+"' THEN CAST((dg___kwh)AS DECIMAL(10,2)) END) AS month1_dg_kwh,SUM(CASE WHEN month = '"+month2+"' THEN CAST((dg___kwh)AS DECIMAL(10,2)) END) AS month2_dg_kwh,SUM(CASE WHEN month = '"+month3+"' THEN CAST((dg___kwh)AS DECIMAL(10,2)) END) AS month3_dg_kwh,SUM(CASE WHEN month = '"+month1+"' THEN CAST((hvac___kwh)AS DECIMAL(10,2)) END) AS month1_hvac_kwh,SUM(CASE WHEN month = '"+month2+"' THEN CAST((hvac___kwh)AS DECIMAL(10,2)) END) AS month2_hvac_kwh,SUM(CASE WHEN month = '"+month3+"' THEN CAST((hvac___kwh)AS DECIMAL(10,2)) END) AS month3_hvac_kwh,SUM(CASE WHEN month = '"+month1+"' THEN CAST((r22___kg)AS DECIMAL(10,2)) END) AS month1_r22_kg,SUM(CASE WHEN month = '"+month2 + \
            ...
            month2+"' THEN CAST((r407___kg)AS DECIMAL(10,2)) END) AS month2_r407_kg,SUM(CASE WHEN month = '"+month3+"' THEN CAST((r407___kg)AS DECIMAL(10,2)) END) AS month3_r407_kg,SUM(CASE WHEN month = '"+month1+"' THEN CAST((other___kg)AS DECIMAL(10,2)) END) AS month1_other_kg,SUM(CASE WHEN month = '" + \
            month2+"' THEN CAST((other___kg)AS DECIMAL(10,2)) END) AS month2_other_kg,SUM(CASE WHEN month = '"+month3 + \
            "' THEN CAST((other___kg)AS DECIMAL(10,2)) END) AS month3_other_kg FROM approved_month WHERE month IN (%s, %s, %s) GROUP BY store_code ORDER BY store_code; "
...
# UPDATE STORE WITH STORE ID
    ...
    def updateFollow(pocname, pocmobile, pocemail, follow1, follow2, follow3, bill_paid, mms, sno, month):
        ...
        cur.execute(sql, [pocname, pocmobile, pocemail, follow1,
                    follow2, follow3, bill_paid, mms, sno, month])
        mysql.connection.commit()
        cur.close()
...
# QUARTERLY TEST
    ...
    # QUARTER 1
        q1 = "SELECT DATE_FORMAT(MIN(`service_from`),'%Y-%m-%d') as service_from, DATE_FORMAT(MAX(`service_to`),'%Y-%m-%d') as service_to, MIN(`service_from`) as service_fromm, MAX(`service_to`) as service_too FROM approved_month WHERE DATE(service_from) >= '" + \
            str(year)+"-04-01' AND DATE(service_to) <= '"+str(year)+"-06-30';"

        q11 = "SELECT `month` FROM approved_month WHERE DATE(service_from) >= '"+str(
            year)+"-04-01' AND DATE(service_to) <= '"+str(year)+"-06-30' GROUP BY `month`;"
    ...
    # QUARTER 2
        q2 = "SELECT DATE_FORMAT(MIN(`service_from`),'%Y-%m-%d') as service_from,DATE_FORMAT(MAX(`service_to`),'%Y-%m-%d') as service_to,MIN(`service_from`) as service_fromm, MAX(`service_to`) as service_too FROM approved_month where service_from >= '" + \
            str(year)+"-07-01' and service_to<='"+str(year)+"-09-30'"

        q22 = "SELECT `month` FROM approved_month WHERE DATE(service_from) >= '"+str(
            year)+"-07-01' AND DATE(service_to) <= '"+str(year)+"-09-30' GROUP BY `month`;"
    ...
    # QUARTER 3
        q3 = "SELECT DATE_FORMAT(MIN(`service_from`),'%Y-%m-%d') as service_from,DATE_FORMAT(MAX(`service_to`),'%Y-%m-%d') as service_to,MIN(`service_from`) as service_fromm, MAX(`service_to`) as service_too FROM approved_month where service_from >= '" + \
            str(year)+"-10-01' and service_to<='"+str(year)+"-12-31'"

        q33 = "SELECT `month` FROM approved_month WHERE DATE(service_from) >= '"+str(
            year)+"-10-01' AND DATE(service_to) <= '"+str(year)+"-12-31' GROUP BY `month`;"
    ...
    # QUARTER 4
        q4 = "SELECT DATE_FORMAT(MIN(`service_from`),'%Y-%m-%d') as service_from,DATE_FORMAT(MAX(`service_to`),'%Y-%m-%d') as service_to,MIN(`service_from`) as service_fromm, MAX(`service_to`) as service_too FROM approved_month where service_from >= '" + \
            str(year1)+"-01-01' and service_to<='"+str(year1)+"-03-31'"

        q44 = "SELECT `month` FROM approved_month WHERE DATE(service_from) >= '"+str(
            year1)+"-01-01' AND DATE(service_to) <= '"+str(year1)+"-03-31' GROUP BY `month`;"
...
# FOLLOW UP BULK UPDATE
    ...
    def followupbulkupdate(follow1, follow2, follow3, billpaid, mms, bulk_update):
        ...
        sql = "UPDATE `user_tables` SET `follow1`= %s, `follow2`= %s, `follow3`= %s, `bill_paid`= %s, `mms`= %s WHERE `id` IN ({})".format(
            placeholders)

        print(sql)
        # cur.execute(sql,[follow1,follow2,follow3,bill_paid,mms],tuple(bulk_update.values()))
        cur.execute(sql, [follow1, follow2, follow3,
                    billpaid, mms] + list(bulk_update))
...
############### FOLLOW UP POC DETAILS UPDATE ###############


    def pocupdate(month1, month2):
        cur = mysql.connection.cursor()
        
        sql = "UPDATE user_tables AS s1 JOIN (SELECT store_code, poc_name, poc_mobile, poc_email FROM user_tables WHERE poc_name <> '' AND poc_mobile <> '' AND poc_email <> '' AND month = %s) AS s2 ON s1.store_code = s2.store_code SET s1.poc_name = s2.poc_name, s1.poc_mobile = s2.poc_mobile, s1.poc_email = s2.poc_email WHERE s1.poc_name = '' AND s1.poc_mobile = '' AND s1.poc_email = '' AND s1.month = %s"
